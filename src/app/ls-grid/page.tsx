...
import { GripVertical, X, Plus, Save, RefreshCw, Grid2X2 } from "lucide-react";
...
function ImageCard({
  ...
  const imgUrl = `https://look-and-seen.vercel.app/work/${filename}`;
  const catColor = CATEGORY_COLORS[project.category] ?? "bg-neutral-700 text-neutral-300";

  return (
    <div className="group relative flex gap-3 items-start rounded-xl border border-[#1A1816] bg-[#0D0C0A] p-3 hover:border-[#B8956A]/30 transition-colors">
      {draggable && (
      ...
      {/* Thumbnail */}
      <div className="shrink-0 w-16 h-16 rounded-lg overflow-hidden bg-[#1A1816]">
        <img
          src={imgUrl}
          alt={project.alt}
      ...
      {/* Position badge */}
      {draggable && (
        <span className="shrink-0 text-[10px] font-mono text-[#4A4540] mt-1">#{index + 1}</span>
      )}
      {/* Remove button */}
      {onRemove && (
        <button
          onClick={onRemove}
          className="absolute top-2 right-2 p-1 rounded-md text-[#6B6560] hover:text-red-400 hover:bg-red-400/10 transition-colors"
...
function NotInGridCard({
  ...
  return (
    <div className="group relative flex gap-3 items-center rounded-xl border border-[#1A1816] bg-[#0A0908] p-3 hover:border-[#B8956A]/20 transition-colors">
      <div className="shrink-0 w-12 h-12 rounded-lg overflow-hidden bg-[#1A1816]">
        <img
          src={imgUrl}
          alt={filename}
  ...
  return (
    <div className="space-y-6">
      ...
      {toast && (
        <div
          className={cn(
            "fixed bottom-6 right-6 z-50 flex items-center gap-3 rounded-xl border px-4 py-3 text-sm shadow-2xl transition-all",
            toast.type === "success"
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold text-[#E8E4DF] font-[family-name:var(--font-syne)]">
                In Grid
              </h2>
              <span className="text-xs text-[#6B6560] bg-[#1A1816] rounded-full px-2 py-0.5">
                {inGrid.length} images
              </span>
            </div>
