/**
 * Grove Prompt Engine — self-scheduling architecture
 *
...
// Multi-tenant: each family has their own Grove URL
// For sweeney.family (the Sweeney's private instance): sweeney.family
// For grove.com (the product, when launched): grove.com
// Stored per-family in grove_families.groveUrl, falls back to env var
function getGroveUrl(familyGroveUrl?: string | null): string {
  return familyGroveUrl ?? process.env.GROVE_BASE_URL ?? "https://grove.com";
}
...
export const kickoffMemberChain = internalMutation({
  ...
  handler: async (ctx, { memberId, familyId, delayDays }) => {
    ...
    await ctx.scheduler.runAt(
      fireAt,
      internal.grovePrompts.sendNextPrompt,
      { memberId, familyId, promptIndex: 0 }
    );

    console.log(`[grove] Chain started for member ${memberId}, fires in ${days}d`);
  },
});
...
export const sendNextPrompt = internalAction({
  ...
  handler: async (ctx, { memberId, familyId, promptIndex }) => {
    ...
    // 1. Load context
    const context = await ctx.runQuery(internal.grovePrompts.getMemberContext, {
      memberId,
      familyId,
    });

    if (!context) {
      console.log(`[grove] Member ${memberId} not found or family missing — chain stopped`);
      return;
    }
    ...
    if (!member.hasAccepted && !member.email) {
      console.log(`[grove] Member ${member.name} has no email and never accepted — chain paused`);
    ...
    const prompt = await ctx.runQuery(internal.grovePrompts.getPromptForIndex, {
      memberId,
    ...
    const lookahead = await ctx.runQuery(internal.groveAI.getStoredPromptsForMember, {
      memberId,
      afterIndex: promptIndex + 1,
    });
    if (lookahead.length < 3) {
      // Non-blocking — runs in background, chain continues regardless
      await ctx.scheduler.runAfter(0, internal.groveAI.generatePromptsForMember, {
        memberId,
        familyId,
      });
      console.log(`[grove] Stock low for ${member.name} — queued AI regeneration`);
    }
    ...
    if (member.email && prompt) {
      emailSent = await sendPromptEmail({
        ...
        familyId,
        promptIndex,
        groveUrl: getGroveUrl((family as { groveUrl?: string }).groveUrl),
      });
    } else if (!member.email) {
      console.log(`[grove] ${member.name} has no email — recording miss, chain continues`);
    }

    // 4. Record this send in the DB
    if (prompt) {
      await ctx.runMutation(internal.grovePrompts.recordPromptSend, {
        memberId,
        familyId,
        promptIndex,
    ...
    console.log(`[grove] Sent prompt ${promptIndex} to ${member.name}, next scheduled in ${PROMPT_INTERVAL_DAYS}d`);
  },
});
...
export const getPromptForIndex = internalQuery({
  ...
  handler: async (ctx, { memberId, relationshipKey, childName, parentNames, index }) => {
    ...
    const { getPromptsForRelationship } = await import("./grove");
...
// Call this instead of inserting into grove_circle directly
// ─────────────────────────────────────────────────────────────────────────────
...
export const addMemberAndStart = mutation({
  ...
  handler: async (ctx, { familyId, name, email, relationshipKey, delayDays }) => {
    ...
    // Insert member
    const memberId = await ctx.db.insert("grove_circle", {
      familyId,
      name,
      email,
      relationshipKey,
...
async function sendPromptEmail({
  ...
  if (!resendKey) {
    console.error("[grove] No RESEND_API_KEY");
    return false;
  }
  ...
  const token = generateToken();
  const submitUrl = `${groveUrl}/respond/${token}`;
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: family.childEmailAlias
          ...
          : `${childFirst} via Grove <hello@grove.com>`,
      to: member.email,
      subject: `${childFirst} has a question for you`,
      html,
      reply_to: family.childEmailAlias ?? `hello@grove.com`,
      tags: [
  ...
  if (!res.ok) {
    ...
    console.error(`[grove] Resend failed for ${member.name}: ${res.status} ${err}`);
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
        ...
        <tr><td align="center" style="padding-bottom:28px;">
          <span style="font-family:Georgia,'Times New Roman',serif;font-size:22px;font-weight:700;color:#B8965A;letter-spacing:-0.01em;">Grove</span>
        </td></tr>
        ...
        <tr><td align="center" style="padding-top:24px;">
          <p style="font-size:11px;color:#C4C0B8;line-height:1.8;margin:0;">
            Grove &middot; ${childFirst}&rsquo;s circle &middot; Set up by ${parentNames}
          </p>
        </td></tr>
