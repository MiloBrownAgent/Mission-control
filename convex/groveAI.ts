/**
 * Grove AI Layer — personalized prompt generation
 *
 * Two jobs:
 *
...
export const generatePromptsForMember = internalAction({
  ...
  handler: async (ctx, { memberId, familyId }) => {
    ...
    const context = await ctx.runQuery(internal.groveAI.getGenerationContext, {
      memberId,
      familyId,
    });

    if (!context) {
      console.error(`[grove-ai] No context for member ${memberId}`);
      return;
    }
    ...
    if (!apiKey) {
      console.error("[grove-ai] No OPENAI_API_KEY — falling back to static library");
      // Graceful degradation: pull from static library
      await ctx.runMutation(internal.groveAI.seedFromStaticLibrary, { memberId, familyId });
      return;
    }
    ...
    const systemPrompt = `You are Grove, a service that helps families preserve memories for their children.
...
Only return valid JSON. No explanation, no markdown fences.`;
    ...
    try {
      ...
      console.log(`[grove-ai] Generated ${prompts.length} prompts for ${member.name}`);
    } catch (err) {
      console.error(`[grove-ai] Generation failed for ${member.name}:`, err);
    ...
    await ctx.runMutation(internal.groveAI.storeGeneratedPrompts, {
      memberId,
      familyId,
      prompts: normalized,
    });
...
Categories: motor | language | social | cognitive | physical | other`,
    ...
    try {
      ...
      await ctx.runMutation(internal.groveAI.recordDetectedMilestones, {
        familyId,
        milestones: detected,
        detectedFromDate: entryDate,
      });

      console.log(`[grove-ai] Detected ${detected.length} milestones from Chronicle entry`);
    } catch {
...
export const getGenerationContext = internalQuery({
  args: { memberId: v.id("grove_circle"), familyId: v.string() },
  handler: async (ctx, { memberId, familyId }) => {
...
export const getStoredPromptsForMember = internalQuery({
  args: { memberId: v.id("grove_circle"), afterIndex: v.number() },
...
export const storeGeneratedPrompts = internalMutation({
  ...
  handler: async (ctx, { memberId, familyId, prompts }) => {
    ...
    for (const prompt of prompts) {
      await ctx.db.insert("grove_generated_prompts", {
        memberId,
        familyId,
  ...
  handler: async (ctx, { familyId, milestones, detectedFromDate }) => {
    for (const m of milestones) {
      ...
      if (existing && !existing.reachedAt) {
        ...
        await ctx.db.insert("grove_milestones", {
          familyId,
          name: m.name,
          category: m.category,
...
export const logMilestone = mutation({
  ...
  handler: async (ctx, { familyId, name, category, note, reachedAt }) => {
    ...
    return await ctx.db.insert("grove_milestones", {
      familyId,
      name,
      category,
      expectedAgeMonths: 0,
